...
while True:
    ...
    if gamemode == "":
        ...
        break
    elif gamemode == "survival" or gamemode == "creative":
        print(f"gamemode={gamemode}\n")
        break
    else:
        print("\nEnter only [survival/creative]")

while True:
    max_player = input("Max Players ?")
    if max_player == "":
        max_player = 20
        print(f"max-players={max_player}\n")
        break
    try:
...
while True:
    ...
    if difficulty == "":
        ...
        break
    elif difficulty in ["easy", "normal", "hard"]:
        print(f"difficulty={difficulty}\n")
        break
    else:
        print("\nEnter only [easy/normal/hard]\n")
...
while True:
    ...
    if online_mode == "":
        ...
        break
    elif online_mode in ["true", "false"]:
        print(f"online-mode={online_mode}\n")
        break
    else:
        print("\nEnter only [true/false]\n")
...
while True:
    ...
    if simulation_distance == "":
        ...
        print(f"simulation-distance={simulation_distance}\n")
        break
    try:
...
# دریافت RAM - بدون حلقه بی‌نهایت
while True:
    ram = input("How much RAM for server? (2, 4): ")
    if ram == "":
        ram = "2G"
        break
    try:
        ...
        if ram_value > 0:
            ...
            print("\nEnter a positive RAM amount\n")
    except ValueError:
        print("\nEnter a valid RAM amount (e.g., 2 or 4)\n")

# تعریف توابع در اینجا (بیرون از هر حلقه)
...
echo   Name: {server_name}           --FmSV--
echo   Version: {selected['id']}     -by ZAW-
echo   RAM: {ramloc}
echo ========================================
...
def set_server_setting():
    ...
    try:
        ...
        with open("server.properties","w") as f:
            f.write(f"max-players={max_player}\n")
            f.write(f"gamemode={gamemode}\n")
            f.write(f"difficulty={difficulty}\n")
            f.write(f"hardcore={hardcore}\n")
            if level_seed:  # فقط اگر seed وارد شده بود
